...
async function initDb() {
  ...
  try {
    // 1. Create tables without foreign keys first to ensure they exist
    await sql`
    ...
    // 2. Add constraints individually using try-catch to avoid blocking if they exist
    ...
    try {
      ...
      // Ignore migration errors as they usually mean columns exist
    }
...
async function startServer() {
  ...
  app.get("/api/users/:id", async (req, res) => {
    ...
    try {
      ...
      // Return 404 on database error to allow demo mode
  ...
  app.get("/api/workers", async (_req, res) => {
    ...
    try {
      ...
      res.json(workers.map(w => ({
        ...
        userId: w.user_id,
        skills: w.skills,
        trustLevel: w.trust_level,
        trustScore: w.trust_score,
        badges: w.badges,
        completedJobsCount: w.completed_jobs_count,
        disputesCount: w.disputes_count,
        avgRating: w.avg_rating,
        availability: w.availability,
        bio: w.bio,
        lat: w.lat,
        lng: w.lng,
        isVouched: w.is_vouched,
        registrationPaid: w.registration_paid
      })));
    } catch (err) {
      console.error(err);
      res.json([]);
  ...
  app.post("/api/workers", async (req, res) => {
    if (!sql) return res.status(503).json({ error: "Database not available" });
    const { 
      userId, skills, experienceYears, trustLevel, trustScore, 
      availability, bio, lat, lng, completedJobsCount, disputesCount, 
      avgRating, isVouched, registrationPaid 
    } = req.body;
    try {
      await sql`
        INSERT INTO worker_profiles (
          user_id, skills, experience_years, trust_level, trust_score, 
          availability, bio, lat, lng, completed_jobs_count, disputes_count, 
          avg_rating, is_vouched, registration_paid
        )
        VALUES (
          ${userId}, ${skills || null}, ${experienceYears || 0}, ${trustLevel || 'new'}, ${trustScore || 0}, 
          ${availability || 'available'}, ${bio || null}, ${lat || null}, ${lng || null}, 
          ${completedJobsCount || 0}, ${disputesCount || 0}, ${avgRating || 0}, 
          ${isVouched || false}, ${registrationPaid || false}
        )
  ...
  app.patch("/api/jobs/:id/status", async (req, res) => {
    ...
    try {
      if (status === 'completed') {
        ...
        `;
        // Increment worker's completed jobs count if workerId is provided
        if (workerId) {
          await sql`
            UPDATE worker_profiles 
        ...
        await sql`
          UPDATE jobs 
          SET status = ${status}, worker_id = COALESCE(${workerId || null}, worker_id)
          WHERE id = ${req.params.id}
        `;
      }
      res.json({ success: true });
  ...
  app.post("/api/reviews", async (req, res) => {
    ...
    try {
      await sql`
        INSERT INTO reviews (id, job_id, reviewer_id, worker_id, rating, comment)
        VALUES (${id}, ${jobId}, ${reviewerId}, ${workerId}, ${rating}, ${comment})
      `;
      ...
      await sql`
        UPDATE worker_profiles
        SET avg_rating = (
          SELECT AVG(rating) FROM reviews WHERE worker_id = ${workerId}
        )
        WHERE user_id = ${workerId}
      `;
  ...
  app.post("/api/messages", async (req, res) => {
    ...
    try {
      await sql`
        INSERT INTO messages (id, job_id, sender_id, content)
        VALUES (${id}, ${jobId}, ${senderId}, ${content})
      `;
      res.json({ success: true });
    } catch (err) {
      res.status(500).json({ error: "Database error" });
  ...
  app.post("/api/vouch", async (req, res) => {
    ...
    try {
      await sql`
        INSERT INTO vouches (voucher_id, worker_id)
        VALUES (${voucherId}, ${workerId})
        ON CONFLICT DO NOTHING
      `;
      
      // Update worker's trust score/vouched status
      ...
      await sql`
        ...
        SET trust_score = trust_score + 5,
            is_vouched = ${count >= 3} -- Auto-vouch after 3 vouches
        WHERE user_id = ${workerId}
      `;
  ...
  // M-Pesa STK Push — real Daraja or simulation fallback
  app.post("/api/mpesa/stk-push", async (req, res) => {
      ...
      try {
        ...
        if (darajaResp.ResponseCode !== "0") {
          ...
          return res.status(400).json({
            error: darajaResp.errorMessage || darajaResp.ResponseDescription || "STK Push rejected"
          });
        }

        if (sql) {
          await sql`
            INSERT INTO transactions (id, job_id, user_id, amount, type, status, mpesa_receipt)
            VALUES (${transactionId}, ${jobId || null}, ${userId}, ${amount}, ${type}, 'pending', ${darajaResp.CheckoutRequestID})
          `;
        }
    ...
    // ── SIMULATION FALLBACK (no Daraja credentials configured) ───────────────
    ...
    try {
      if (sql) {
        await sql`
          INSERT INTO transactions (id, job_id, user_id, amount, type, status, mpesa_receipt)
          VALUES (${transactionId}, ${jobId || null}, ${userId}, ${amount}, ${type}, 'completed', ${fakeReceipt})
        `;
        if (type === "registration") {
          await sql`
            UPDATE worker_profiles
            SET registration_paid = true, trust_score = trust_score + 10
  ...
  // Safaricom API Gateway IPs (from Daraja docs — whitelist these)
  ...
  // Daraja payment callback — Safaricom POSTs here after customer confirms / cancels
  app.post("/api/mpesa/callback", async (req, res) => {
    // In production, only accept from Safaricom's gateway IPs
    ...
    try {
      ...
      if (resultCode === 0) {
        ...
        await sql`
          UPDATE transactions
          SET status = 'completed', mpesa_receipt = ${mpesaReceipt}
          WHERE mpesa_receipt = ${checkoutRequestId}
        `;

        // Unlock registration if applicable
        const txRows = await sql`
          SELECT user_id, type FROM transactions WHERE mpesa_receipt = ${mpesaReceipt} LIMIT 1
        `;
        if (txRows.length > 0 && txRows[0].type === "registration") {
          await sql`
            UPDATE worker_profiles
            SET registration_paid = true, trust_score = trust_score + 10
            WHERE user_id = ${txRows[0].user_id}
          `;
        }
  ...
  // Poll payment status — queries Daraja directly when credentials present,
  // so this works even without a configured MPESA_CALLBACK_URL (no ngrok needed in dev).
  app.get("/api/mpesa/status/:checkoutRequestId", async (req, res) => {
    ...
    if (hasDaraja() && !checkoutRequestId.endsWith("_SIM")) {
      try {
        ...
        if (queryResp.ResultCode === "0") {
          // Sync DB if callback hasn't fired yet
          if (sql) {
            await sql`
              UPDATE transactions SET status = 'completed'
              WHERE mpesa_receipt = ${checkoutRequestId} AND status = 'pending'
            `;
            // Unlock registration if applicable
            const txRows = await sql`
              SELECT user_id, type FROM transactions
              WHERE mpesa_receipt = ${checkoutRequestId} AND type = 'registration' LIMIT 1
            `;
            if (txRows.length > 0) {
              await sql`
                UPDATE worker_profiles
                SET registration_paid = true, trust_score = trust_score + 10
                WHERE user_id = ${txRows[0].user_id} AND registration_paid = false
              `;
            }
          }
        ...
        if (queryResp.ResultCode !== undefined) {
          // Any non-zero result code = terminal failure
          if (sql) {
            await sql`UPDATE transactions SET status = 'failed' WHERE mpesa_receipt = ${checkoutRequestId} AND status = 'pending'`;
          }
          return res.json({ status: "failed", reason: queryResp.ResultDesc });
        }
        // ResponseCode non-zero or no ResultCode yet = still processing
    ...
    // DB fallback (simulation mode, or Daraja query errored)
    if (!sql) return res.json({ status: "unknown" });
    try {
      const rows = await sql`
        SELECT status, mpesa_receipt, amount, type
        FROM transactions
        WHERE mpesa_receipt = ${checkoutRequestId}
      ...
      res.json({
        ...
        mpesaReceipt: rows[0].mpesa_receipt,
  ...
  // Sandbox C2B simulation — triggers a test payment without a real phone
  ...
  // Asante Drop donations (MiniPay / cKES on Celo)
  app.post("/api/donations", async (req, res) => {
    ...
    try {
      if (sql) {
        await sql`
          INSERT INTO donations (id, job_id, user_id, donor_address, amount_kes, tx_hash, token_symbol, status)
          VALUES (${id}, ${jobId || null}, ${userId || null}, ${donorAddress || null}, ${amountKES}, ${txHash || null}, ${tokenSymbol || 'cKES'}, 'confirmed')
  ...
  app.get("/api/donations", async (_req, res) => {
    ...
    try {
      const rows = await sql`
        SELECT id, job_id, donor_address, amount_kes, tx_hash, token_symbol, created_at
