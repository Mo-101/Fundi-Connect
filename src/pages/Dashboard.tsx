...
function ClientDashboard({ user, jobs, nearbyWorkers, transactions, onStart, onComplete, onReport }: { user: User, jobs: Job[], nearbyWorkers: (User & WorkerProfile)[], transactions: any[], onStart?: (job: Job) => void, onComplete: (job: Job) => void, onReport: (id: string) => void }) {
  const navigate = useNavigate();
  ...
  const filteredWorkers = nearbyWorkers.filter(worker => {
    ...
    const matchesSkill = worker.skills?.some(skill => skill.toLowerCase().includes(q));
    const matchesLocation = worker.location?.toLowerCase().includes(q);
    const matchesName = worker.name?.toLowerCase().includes(q);

    return matchesSkill || matchesLocation || matchesName;
  });
  return (
    <div className="space-y-12 pb-10">
       <header className="flex justify-between items-center px-1">
      ...
      <section className="space-y-8 px-1">
        ...
        {nearbyWorkers.length > 0 && (
          <div className="relative mx-3">
            ...
            <input
              ...
              className="w-full bg-stone-50 border border-stone-200 text-stone-950 rounded-[24px] py-3.5 pl-12 pr-10 text-xs font-black uppercase tracking-wider focus:outline-none focus:border-brand-indigo/60 focus:bg-white focus:ring-4 focus:ring-brand-indigo/10 transition-all placeholder:text-stone-300"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute inset-y-0 right-0 flex items-center pr-4 text-xs font-black uppercase tracking-widest text-stone-400 hover:text-brand-red active:scale-95 transition-all"
              >
                Clear
              </button>
            )}
        ...
        {nearbyWorkers.length === 0 ? (
          ...
          <div className="text-center py-16 bg-stone-50 rounded-[48px] border border-stone-200 mx-3">
            ...
            <p className="text-stone-500 serif italic text-md leading-none">No fundis found for "{searchQuery}"</p>
            <p className="text-stone-400 text-[9px] mt-2 font-black uppercase tracking-widest">Try searching for other skills or regions</p>
          </div>
        ) : (
            ...
            <div className="grid gap-4">
              {jobs.map((job) => (
                <div key={job.id} className="bg-white p-6 rounded-[36px] shadow-sm border border-stone-50 flex items-center justify-between group active:scale-[0.98] transition-all">
                  ...
                  <div className="flex items-center gap-3 gap-y-1.5 flex-wrap justify-end">
                    {job.status === 'accepted' && onStart && (
                      <button 
                        onClick={(e) => { e.stopPropagation(); onStart(job); }}
                        className="px-6 py-2.5 bg-brand-indigo text-white text-[9px] font-black uppercase tracking-[0.2em] rounded-xl shadow-lg active:scale-95 transition-all whitespace-nowrap"
                      >
                        Start
                      </button>
                    )}
                    {job.status === 'in_progress' && (
                      <button 
                        onClick={(e) => { e.stopPropagation(); onComplete(job); }}
                        className="px-6 py-2.5 bg-brand-red text-white text-[9px] font-black uppercase tracking-[0.2em] rounded-xl shadow-lg active:scale-95 transition-all whitespace-nowrap"
                      >
                        Complete
                      </button>
                    )}
